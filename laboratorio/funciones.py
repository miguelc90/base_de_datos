...
#funcion para los inputs de cada case en las funcioens actulizar_alumnos y datos_alumnos
############################################################# FALTAN VALIDACIONES ######################################################################## 
def nuevo_registro():
    while True:
...
def opcion_actualizacion_completa(id_alumno):
    ...
    try:
        ...
        if registro_alumnos and registro_datos:
            ...
            ############ FALTAN VALIDACIONES CORRESPONDIENTES ##############
            nombre = validar_cadena(input("Nuevo nombre: "))
            apellido = validar_cadena(input("Nuevo apellido: "))
            dni = pedir_dni()
            ...
            telefono = validar_numero(input("Nuevo teléfono: "))
...
def actualizar_Alumnos(parametro):
    ...
    match parametro:
        ...
        case 3:
            query += " dni = %s"
            nuevo_valor = nuevo_registro_val(validar_digito_campos,8,8,input("Ingresar DNI: "),"DNI")
            while True:
                if dni_existente(nuevo_valor):
...
def actualizar_datos_alumnos(parametro):
    ...
    match parametro:
        case 4:
            query += " domicilio = %s"
            nuevo_valor = input("Ingresar Domicilio: ").title()
            while True:
                if len(nuevo_valor)>50 or len(nuevo_valor)==0:
                    print("Error, el campo domilicio no puede quedar vacío, ni superar los 50 carácteres\n")
                    nuevo_valor=input("Ingresar Domicilio: ")
                else:
                    break
        case 5:
            query += " telefono = %s"
            nuevo_valor = nuevo_registro_val(validar_digito_campos,12,12,input("Ingresar Teléfono: ","Teléfono"))
        case 6:
...
def eliminar(id_alumno):
    ...
    try:
        ...
        if not resultado:
            ...
            mostrar_coincidencia_id(cursor, id_alumno)
            decision = validar_numero(input("Para confirmar la eliminación resuelva el calculo matemático\n3+3: "))
            if decision == '6':
    ...
    try:
        ...
        match consulta:
            case 1:
                ...
                print(f"{"id":<3} {"Nombre":<15} {"Apellido":<15} {"DNI":<10}  {"Domicilio":20} {"Telefono":<13} {"Fecha de nacimiento"}")
                print()
                for fila in resultado:
                    print(f"{fila[0]:<3} {fila[1]:<15} {fila[2]:<15} {fila[3]:<10} {fila[6]:<20} {fila[7]:<13} {fila[8]}") 
            case 2:
                # SI LA OPCIÓN ES 2 SE CONSULTA SOLO LOS DATOS DE LA TABLA ALUMNOS
                cursor.execute('SELECT * FROM gestion_alumnos.alumnos')
                resultado=cursor.fetchall()
                print(f"{"id":<3} {"Nombre":<15} {"Apellido":<15} {"DNI":<10}")
                print()
                for fila in resultado:
                    print(f"{fila[0]:<3} {fila[1]:<15} {fila[2]:<15} {fila[3]:<10}")  
            ...
            case 3:
                ...
                resultado=cursor.fetchall()
                print(f"{"Id_Alumno":<10} {"Domicilio":<20} {"Teléfono":<13} {"Fecha de nacimiento"}")
                print()
                for fila in resultado:
                    print(f"{fila[1]:<10} {fila[2]:<20} {fila[3]:<13} {fila[4]}") 
    except mysql.connector.Error as e:
    ...
    while True:
        if len(dato) < limiteMin or len(dato) > limiteMax:
            if limiteMin==limiteMax:
                print(f"El campo {nombre_dato} no puede qudar vacío, y tiene que ser exactamente 8 dígitos")
...
def insertar_registro():
    ...
    while decision.lower() == 'si':
        ...
        nombre = validar_cadena(input("Ingrese su nombre: "))
        apellido = validar_cadena(input("Ingrese su apellido: "))
        dni = pedir_dni()
        direccion = validar_direccion()
        telefono = validar_numero(input("Ingrese su numero de telefono: "))
        fecha_de_nacimiento = validar_fecha_nacimiento()
